...
## СТОП-сигнал: коммит
Перед ЛЮБЫМ git commit после работы с кодом:
1. Критик запускался? → Были P0/P1? → ДА → Записаны в tasks.json log? → НЕТ → СТОП, запиши.
2. errors_since_retro >= 3? → Ретроспектива проведена? → НЕТ → СТОП.
3. Пользователю показан отчёт (таблица ниже)? → НЕТ → СТОП, покажи.
Нарушение = ошибка протокола → запись в agent-memory/lessons.md.

## Отчёт после критика (ПОКАЗАТЬ ПОЛЬЗОВАТЕЛЮ)
После каждого запуска критика — вывести таблицу:

| # | Проблема | Виновник | Приоритет | Решение | Статус |
|---|----------|----------|-----------|---------|--------|
| 1 | описание | agent    | P0/P1/P2  | что сделано | FIXED/SKIP/DEFERRED |

Затем:
- errors_since_retro: X → Y
- Ретроспектива: нужна/не нужна/проведена
- Уроки: добавлены в [agent] / не нужны
