...
1. Что на входе у этой персоны? → 2. Что произойдёт в коде? → 3. Что получит пользователь?
Если результат неполный (нет fallback, потеряны данные, не учтён контекст) — допиши.
